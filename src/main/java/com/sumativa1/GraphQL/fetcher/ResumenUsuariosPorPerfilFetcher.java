...
import com.sumativa1.GraphQL.mock.MockDatabase;
...
public class ResumenUsuariosPorPerfilFetcher implements DataFetcher<List<Map<String, Object>>>{
    ...
    //---------  Descomentar luego del mock, cuando el back esté arriba ---------//
    ...
    public List<Map<String, Object>> get(DataFetchingEnvironment environment) {
        return MockDatabase.getUsuarios().stream()
            .collect(Collectors.groupingBy(user -> (String) user.get("perfil"), Collectors.counting()))
            .entrySet().stream()
            .map(entry -> {
                Map<String, Object> resultado = new HashMap<>();
                resultado.put("perfil", entry.getKey());
                resultado.put("totalUsuarios", entry.getValue().intValue());
                return resultado;
            })
